...
**点击 ▶ 进入 Listen 那一刻**: **房间里那扇窗户**机械合上 800ms → 外面雨声变小变闷 → 房间灯变暗 → 搜索 / 队列 / 推荐这些 Browse-only chrome 从窗台下方收掉 → 唱片机从窗台升起 → 歌词带从下方浮出 → DJ 呵气云在屏幕左下角凝结。

整套动画用来表达"我把外面的世界关掉了"。这是产品差异点的视觉化。

**永远不变的事实**: 整个 UI 是一间小房间。窗户是房间中上区域的一个**元素**,**不会变成整屏**。无论 Browse 还是 Listen,你都能看到窗框 / 窗台 / 墙 / 灯。Listen 只是把窗户**关上**+ 把窗台下方的"今日推荐 / 队列 / 搜索"换成"唱片 / 歌词 / viz"。
...
### 3.3 Listen 是房间的一种状态,不是覆盖层

Listen **不覆盖**整个 UI。它只**改变房间内的两件事**:窗户从开到关 + 下半窗台从"papers"切到"vinyl/lyrics/viz"。**房间本身 (墙 / 窗框 / 窗台 / 灯) 一直在,只是灯变暗 30%**。

Listen **不参与**左右上下 5 view 导航。它只是 Browse 的一个状态分支 (窗开 vs 窗关)。Chat / Taste / Today 切换时,如果在 Listen 里,先回 Browse 再切。
...
之前 PRD 隐含 "Listen 时窗户 = 整个 UI",**这是错的**。修正后的正确设定:

- **整个 UI 永远是一间小房间**。墙 / 窗框 / 窗台 / 灯 在所有模式都看得到
- **窗户是房间中上区域的一个元素**,**不变成整屏**
- Browse: 房间内,窗户**开着**,看到外面 RainEngine。下半窗台摆**搜索 / 队列 / 推荐**
- Listen: 房间内,窗户**关上**,看到雨打在窗玻璃上凝结大水珠。下半窗台摆**唱片 / 歌词 / viz + DJ 云**
- 唱片 / 歌词 / viz **全部在下半视口**,**不抢窗户的主视角**

### 4.1 整体 ASCII mockup (Listen 模式,窗关)
...
**关键比例 (vh)**:
- 0-8vh:    顶部 Claudio + 时间 + ⚙ (小,留白)
- 8-55vh:   窗框 + 窗户 (上半主体)
- 55-58vh:  窗台 (raised wood hairline)
- 58-78vh:  唱片 + 歌名信息行
- 78-88vh:  歌词带 (3 行)
- 88-98vh:  viz + 退出 + DJ 云
- 98-100vh: 留白
...
**房间不动,只是窗户从开切到关 + 下半窗台内容物切换**。

```text
t=0    用户点队列里某首歌的 ▶, 或者搜索结果某行的 ▶
t=0    sill 上 Browse papers (搜索/队列/推荐) fade + translateY +12 (300ms, stagger 50)
t=200  房间灯泡 brightness 100%→70% (800ms 缓变)
t=200  窗外 RainEngine 视差速度降 50%
t=200  窗户**机械合上** (上下推拉窗,上半截下沿 translateY 0→100%, 800ms `--ease-mech`)
t=400  RainEngine ⇌ RainOnGlassEngine cross-fade (在窗框内部画布交叉淡化)
t=500  外面雨声音量降 50% + 切到"打玻璃"音色 (闷响)
t=700  唱片从窗台后方 translateY +80 + rotate(-10deg) + opacity 0 升起 (spring)
t=800  窗户合完 (有可选 50ms 木头碰撞音效)
t=800  音乐 audio.play()
t=800  viz bar opacity 0→0.7
t=1000 歌词带从下方 +24 浮出
t=R    (后端 dj/say 返回) 呵气云在左下角 fade up
```
...
t=100  外面雨声音量 + 音色恢复
t=100  房间灯泡 brightness 70%→100%
t=100  窗户**打开** (上半截下沿 translateY 100%→0%, 600ms `--ease-mech`)
t=300  sill 上 Browse papers 从下方升起 (200ms, stagger 50)
t=600  全部就位
```

**强调**:任何时候都没有"全屏即玻璃"。窗户只是房间中上区域那扇,**它的开关 = clip 上半截木窗框内的画布**。窗框本身始终在视口同一位置。
...
- 2026-05-27 v0.1:
  ...
  - 多轮对话定:
    ...
    - 加唱片机 + viz bars
- 2026-05-27 v0.1.1 (本次更新): 用户提醒 PRD 在 §4.0 和 §3.3 出现 conceptual 错误,把 "Listen = 整屏即玻璃" 当成默认。**修正**:
  - 整个 UI 永远是一间小房间,窗户只是房间中上区域的元素,**不变成整屏**
  - Listen 模式只是房间内"窗户从开到关 + 下半窗台内容物切换",不是覆盖层
  - 所有 view 都看得到房间(墙/窗框/窗台/灯)
  - 唱片/歌词/viz **必须在下半视口**,**不抢窗户主视角**
  - 修了 §1.2 / §3.3 / §4.0 / §4.1 / §5.1 / §5.2 六处描述
